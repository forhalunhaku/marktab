...
function safe(value, secrets) {
  ...
  for (const secret of secrets.filter(Boolean).map(String).sort((a, b) => b.length - a.length)) result = result.replaceAll(secret, '***');
  return result;
}
...
export async function uploadPackage({ productId, clientId, apiKey, zipBytes, fetchImpl = fetch, signal }) {
  const result = await request({ action: 'Edge package upload', url: url(productId, '/submissions/draft/package'), init: { method: 'POST', headers: authHeaders(clientId, apiKey, { 'content-type': 'application/zip' }), body: zipBytes }, fetchImpl, secrets: [clientId, apiKey], expectedStatus: 202, signal });
...
export async function publishSubmission({ productId, clientId, apiKey, notes, fetchImpl = fetch, signal }) {
  const result = await request({ action: 'Edge review submission', url: url(productId, '/submissions'), init: { method: 'POST', headers: authHeaders(clientId, apiKey, { 'content-type': 'application/json' }), body: JSON.stringify({ notes }) }, fetchImpl, secrets: [clientId, apiKey], expectedStatus: 202, signal });
...
export async function waitForOperation({ productId, clientId, apiKey, operationId, kind, fetchImpl = fetch, sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), maxAttempts = 60, intervalMs = 5000, signal }) {
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (result?.status === 'Failed') throw new Error(`Edge ${kind} failed: ${JSON.stringify({ message: result.message, errorCode: result.errorCode, errors: result.errors })}`);
    if (result?.status !== 'InProgress') throw new Error(`Edge ${kind} returned unexpected status: ${result?.status ?? 'missing'}`);
