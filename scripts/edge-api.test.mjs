...
const response = (status, data = null, location = null) => new Response(data === null ? '' : JSON.stringify(data), { status, headers: location ? { location } : {} });

test('uploads with Edge v1.1 credentials', async () => {
  let request;
  const id = await uploadPackage({ productId: 'product', clientId: 'client', apiKey: 'key', zipBytes: Buffer.from('zip'), fetchImpl: async (target, init) => { request = { target, init }; return response(202, null, 'operation-1'); } });
...
test('submits certification notes', async () => {
  let body;
  const id = await publishSubmission({ productId: 'product', clientId: 'client', apiKey: 'key', notes: 'MarkTab 2.0.4', fetchImpl: async (_target, init) => { body = init.body; return response(202, null, '/operations/publish-1'); } });
  assert.equal(id, 'publish-1');
  assert.deepEqual(JSON.parse(body), { notes: 'MarkTab 2.0.4' });
});
