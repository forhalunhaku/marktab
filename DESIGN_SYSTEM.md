...
> 它是所有新项目和跨项目重构的视觉基线。
> 每个项目可以在根目录维护自己的 `DESIGN.md` 作为该项目的具体约束（页面类型、组件清单等）。
> 标准主题遵循森林绿基线；本文明确登记的项目变体属于规范的一部分。项目文档不得引入未登记的新视觉方向。
> 如果项目 DESIGN.md、Agent 指令与代码冲突，先保持已发布产品的一致性，再最小幅度修正文档或实现。
...
| **marktab** | 浏览器新标签页扩展（Mist Blue 效率工具变体，见 3.5） |
...
> 标准主题使用上述森林绿基线。MarkTab 使用本文 3.5 节登记的 Mist Blue 变体，不使用旧的暖纸/sage green 方案。
...
标准主题阴影偏绿色（`rgba(37,58,43,...)`），不用纯黑重阴影。登记变体可以使用与自身背景匹配的低透明度冷暖阴影；MarkTab 使用 `rgba(15,23,42,...)`。

### 3.4 色彩使用硬规则

1. **每个主题只使用一个强调色家族**；标准主题为森林绿，登记变体使用其定义的单一强调色
2. **不用多色标签系统**（如 success 绿 / warning 黄 / error 红三色徽章）
3. **不用亮红**做危险操作；删除用陶土色 `#c97c5d`
4. 所有颜色从 CSS 变量引用，不硬编码 `#333` / `#f0f0f0`
5. 需精细透明度时（遮罩、阴影、叠加层）用 `rgba()`，但优先取自变量，如 `rgba(47,111,78,0.08)` 对应标准主题 `--green`

### 3.5 MarkTab：Mist Blue 效率工具变体

MarkTab 是 HALUNHAKU 的冷灰雾蓝分支。它保留圆润、轻盈、克制、衬线标题和高可读性，但用单一蓝色强调家族替代森林绿。
...
--accent-border: rgba(79,124,255,0.20);

/* Dark */
--dark-bg: #080d16;
--dark-surface: rgba(15,23,42,0.72);
...
1. 只使用蓝色强调家族，不新增绿色、紫色、橙色等第二强调色，也不提供强调色选择器。
2. 阴影使用低透明度 cool slate：`rgba(15,23,42,0.04-0.10)`；深色模式使用 `rgba(2,6,23,0.18-0.46)`。
3. 卡片和控件可以使用半透明白色/深 slate 表面，但仅固定侧栏和搜索遮罩允许 `backdrop-filter`。
4. 渐变只允许用于低对比环境背景、品牌/图标表面和 popup 主按钮；禁止彩虹、高饱和、多色或文字渐变。
5. 运行界面不得加载远程字体、图片或装饰资源；字体和图标必须使用系统栈或扩展包内资源。
6. 运行代码的 token 以 `styles.css` 为准；新增组件应复用 `--accent*`、`--line*`、`--shadow*`、`--radius*`，不要恢复旧的 `--green*` token。
...
```css
@media (max-width: 1440px) { /* 宽屏网格收缩 */ }
@media (max-width: 1200px) { /* 固定侧栏切换为抽屉 */ }
@media (max-width: 1024px) { /* 平板间距与网格 */ }
@media (max-width: 768px)  { /* 手机导航与双列布局 */ }
@media (max-width: 640px)  { /* 紧凑控件 */ }
@media (max-width: 520px)  { /* 单列布局 */ }
```
...
MarkTab 例外：桌面使用固定左侧栏；`1200px` 及以下统一切换为左侧抽屉，触发按钮必须保留可访问名称和清晰焦点态。
...
MarkTab 不使用内容型占位插画；favicon 加载失败时回退为站点首字母图标。
...
- ❌ 多色、高饱和或文字渐变；MarkTab 仅允许 3.5 节定义的克制渐变范围
- ❌ 无范围约束的玻璃拟态；标准主题仅限导航和弹窗遮罩，MarkTab 仅限固定侧栏和搜索遮罩
- ❌ 纸纹/水彩/噪点纹理背景
- ❌ Emoji 作为分类图标
...
1. **不要引入未登记的新视觉风格**：不擅自加玻璃拟态、霓虹色、渐变文字、粒子背景
2. **不要随意新增颜色**：所有颜色用 CSS 变量；新增需先在变量和项目设计文档中定义，且不得形成第二强调色家族
3. **不要重阴影/大渐变/多色系统**：保持轻盈、单色强调、淡阴影
4. **不要破坏阅读体验**：720px 宽度、1.9 行高、衬线标题、无干扰装饰
5. **后台不做独立设计系统**：使用项目同款 token（标准主题为绿色；登记变体使用其单一强调色）
...
| marktab | `AGENTS.md` + `styles.css` | Mist Blue 效率工具变体 | 浏览器扩展，无框架；单一蓝色强调；1200px 抽屉断点 |
