...
1. 在 GitHub Releases 中下载最新的 `marktab-1.4.3.zip`。
...
| `npm run package` | 先执行校验，再生成 `dist/marktab-1.4.3.zip`。 |
...
| `npm run bump -- 1.4.3` | 同步更新 `package.json`、`manifest.json`、README、提交说明和弹窗版本号。 |
...
生成的 zip 位于 `dist/marktab-1.4.3.zip`。请确认压缩包只包含运行所需的 `manifest.json`、HTML、CSS、JS、`_locales/` 和图标文件。Chrome Web Store 提交说明见 [CHROME_STORE_SUBMISSION.md](./CHROME_STORE_SUBMISSION.md)。
